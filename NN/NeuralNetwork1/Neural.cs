...
namespace NeuralNetwork1
{
    ...
    public class Sample
    {
        ...
        /// </summary>
        public AutoType actualClass;

        /// <summary>
        /// Распознанный класс - определяется после обработки
        /// </summary>
        public AutoType recognizedClass;
        ...
        public Sample(Bitmap b, double[] inputValues, int classesCount, AutoType sampleClass = AutoType.Undef)
        {
            bitmap = b;
            //  Клонируем массивчик
            input = (double[]) inputValues.Clone();
            Output = new double[classesCount];
            if (sampleClass != AutoType.Undef) Output[(int) sampleClass] = 1;

            recognizedClass = AutoType.Undef;
        ...
        /// </summary>
        public AutoType ProcessPrediction(double[] neuralOutput)
        {
            Output = neuralOutput;
            if (error == null)
            ...
            for (int i = 0; i < Output.Length; ++i)
            {
                ...
                if (Output[i] > Output[(int) recognizedClass]) recognizedClass = (AutoType) i;
            }
